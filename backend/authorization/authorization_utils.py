...
async def _authorize_token(token: str, token_type: EnpointAuthType, return_user: bool) -> User | None:
    ...
    async with await MainServiceContextManager[MainServiceAuth].create(MainServiceType=MainServiceAuth, postgres_session=session) as auth:
        return await auth.authorize_token_and_optionally_return_user(token=token, token_type=token_type, return_user=return_user)


@endpoint_exception_handler
async def authorize_private_endpoint(
    token: str = Header(..., title="Authorization access/refresh token", examples="Bearer {token}"),
) -> User:
    """Use with fastAPI Depends() in private endpoint"""
    return await _authorize_token(token=token, token_type="access", return_user=True)

@endpoint_exception_handler
async def authrorize_public_endpoint(
    token: str = Header(..., title="Optional authorization access token", examples="Bearer {token}"),
) -> User:
    """Use with fastAPI Depends() in public endpoints"""
    return await _authorize_token(token=token, token_type="optional-access", return_user=True)

@endpoint_exception_handler
async def authorize_password_recovery_endpoint(
    token: str = Header(..., title="Authorization password recovery token", examples="Bearer {token}"),
) -> User:
