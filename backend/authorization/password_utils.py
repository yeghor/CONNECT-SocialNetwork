...
def hash_password(raw_pass: str) -> str:
    try:
        salt, bytes = bcrypt.gensalt(), raw_pass.encode()
        hashed_password = bcrypt.hashpw(bytes, salt)
        return hashed_password.decode()
    except Exception as e:
        raise BcryptError(f"Bcrypt: password encoding failed.") from e
...
def check_password(entered_pass: str, hashed_pass: str) -> bool:
    try:
        ...
        raise BcryptError("Bcrypt: passwords comparing failed.") from e
