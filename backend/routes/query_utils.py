...
def query_prompt_required(query: str = Query(..., max_length=QUERY_PARAM_MAX_L)):
    ...
    if not query.strip():
        raise BadRequestExc(
            dev_log_detail=f"QeryPromptValidator (query_utils): Received empty query prompt.",
            client_safe_detail=f"Prompt can't be empty",
        )
    return query
...
def page_validator(page: int):
    if not page >= 0:
        raise BadRequestExc(
            dev_log_detail=f"PageValidator (query_utils): Received page value that is less or equal than 0.",
            client_safe_detail=f"Invalid page value",
        )
    return int(page)
