...
class S3Storage(ImageStorageABC):
    ...
    def __init__(self, mode: Literal["prod", "test"]):
        ...
        if mode == "prod":
            ...
            raise ValueError("S3 Storage: Unsupported running mode")
        ...
        async with self._client() as s3:
            ...
            if not self._validate_image_mime(
                ...
                raise InvalidFileMimeType(
                    f"S3 Storage: Invalid image type. Allowed only - {ALLOWED_EXTENSIONS}"
                )

            if not self._validate_file_size(bytes_obj=contents):
                raise InvalidResourceProvided(
                    f"S3 Storage: Image is too big. Size up to {POST_IMAGE_MAX_SIZE_MB}mb"
                )
    ...
    async def upload_avatar_user(
        self, contents: bytes, mime_type: str, image_name: str
    ) -> None:
        async with self._client() as s3:
            if not self._validate_image_mime(
                image_bytes=contents, specified_mime=mime_type
                ...
                raise InvalidFileMimeType(
                    detail=f"S3 Storage: User {image_name} tried to upload avatar with wrong mime type",
                    client_safe_detail=f"Invalid image type. Allowed only - {ALLOWED_EXTENSIONS}",
                )

            if not self._validate_file_size(bytes_obj=contents):
                raise InvalidResourceProvided(
                    detail=f"S3 Storage: User {image_name} tried to uploaded avatar bigger than {POST_IMAGE_MAX_SIZE_MB}mb",
    ...
    async def delete_avatar_user(self, user_id: str) -> None:
        async with self._client() as s3:
            try:
                await s3.delete_object(Bucket=self._bucket_name, Key=user_id)
            except Exception as e:
                raise MediaError(
                    f"S3 Storage: Failed to delete user avatar: {e}"
                ) from e
        ...
        async with self._client() as s3:
            ...
            for image_name in images_names:
                try:
                    ...
                    raise MediaError(
                        f"S3 Storage: Failed to get post images presigned URLs. Images names: {images_names}. Exception: {e}"
                    ) from e
    ...
    async def get_user_avatar_url(self, image_name: str) -> str:
        async with self._client() as s3:
            try:
                ...
                raise MediaError(
                    f"S3 Storage: Failed to get user avatar presigned. Image name - {image_name}. Exception: {e}"
                ) from e
...
class LocalStorage(ImageStorageABC):
    ...
    async def upload_images_post(
        ...
        await self._validate_image(
            contents=contents, content_type=content_type, image_name=image_name
        )
        extension = await self._get_extension(
            content_type=content_type, image_name=image_name
        )

        try:
            async with aiofiles.open(
                file=f"{self.__media_post_path}/{image_name}{extension}", mode="wb"
            ) as file_:
                await file_.write(contents)
        except Exception as e:
            raise MediaError(
                f"Local Storage: Failed write post image localy. Image name - {image_name}. Exception - {e}"
            ) from e
    ...
    async def upload_avatar_user(
        ...
        try:
            async with aiofiles.open(
                file=f"{self.__media_avatar_path}/{image_name}{extension}", mode="wb"
            ) as file_:
                await file_.write(contents)
        except Exception as e:
            raise MediaError(
                f"Local Storage: Failed write user avatar localy. Image name - {image_name}. Exception - {e}"
            ) from e
