...
class User(Base):
    ...
    posts: Mapped[List["Post"]] = relationship(
        "Post", back_populates="owner", lazy="selectin"
    )

    actions: Mapped[List["PostActions"]] = relationship(
        "PostActions", back_populates="owner", lazy="selectin"
    )
...
class Post(Base):
    ...
    parent_post_id: Mapped[str] = mapped_column(
        ForeignKey("posts.post_id", ondelete="SET NULL"), nullable=True
    )
    ...
    images: Mapped[List["PostImage"]] = relationship("PostImage", lazy="selectin")
...
class ChatRoom(Base):
    ...
    participants: Mapped[List[User]] = relationship(
        "User", secondary="userroom", back_populates="chat_rooms", lazy="selectin"
    )
...
class UserRoom(Base):
    ...
    room_id: Mapped[str] = mapped_column(
        ForeignKey("chatroom.room_id", ondelete="CASCADE"), primary_key=True
    )
