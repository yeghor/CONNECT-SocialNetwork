...
class PostgresService:
    ...
    async def get_fresh_posts(
        self, user: User | None, page: int, n: int, exclude_ids: List[str]
    ) -> List[Post]:
        if user:
            filtering_stmt = and_(
                Post.owner_id != user.user_id, Post.post_id.not_in(exclude_ids)
            )
        else:
    ...
    async def delete_post_by_id(self, id_: str) -> None:
        await self.__session.execute(delete(Post).where(Post.post_id == id_))
    ...
    async def get_followed_posts(
        self, user: User, n: int, page: int, exclude_ids: List[str] = []
    ) -> List[Post]:
        ...
        result = await self.__session.execute(
            select(Post)
            .where(
                and_(Post.owner_id.in_(followed_ids), Post.post_id.not_in(exclude_ids))
            )
            .order_by(Post.popularity_rate.desc(), Post.published.desc())
            .options(selectinload(Post.parent_post))
            .offset(page * n)
    ...
    async def update_post_fields(
        self, post_data: PostDataSchemaID, return_updated_post: bool = False
    ) -> Post | None:
        post_data_dict = post_data.model_dump(
            exclude_defaults=True, exclude_none=True, exclude={"post_id"}
        )
    ...
    async def get_actions(
        ...
        result = await self.__session.execute(
            ...
            .where(
                and_(
                    PostActions.owner_id == user_id,
                    PostActions.action == action_type,
    ...
    async def get_post_action_by_type(
        self, post_id: str, action_type: ActionType
    ) -> List[PostActions]:
        result = await self.__session.execute(
            select(PostActions)
            .where(
                and_(PostActions.post_id == post_id, PostActions.action == action_type)
            )
            .order_by(PostActions.date.desc())
        )
        return result.scalars().all()
    ...
    async def get_user_actions(
        ...
        n_most_fresh: int | None,
        return_posts: bool = False,
    ) -> List[PostActions] | List[Post]:
        result = await self.__session.execute(
            select(PostActions)
            .where(
                and_(PostActions.owner_id == user_id, PostActions.action == action_type)
            )
            .order_by(PostActions.date.desc())
            .limit(
                n_most_fresh
        ...
        if return_posts:
            return [action.post for action in actions]
        else:
            return actions
    ...
    async def get_post_replies(self, post_id: str, page: int, n: int) -> List[Post]:
        ...
        result = await self.__session.execute(
            ...
            .order_by(
                Post.published.desc(), Post.popularity_rate.desc(), likes_subq.desc()
            )
            .options(selectinload(Post.parent_post))
            .offset(page * n)
            .limit(n)
    ...
    async def get_user_posts(
        self, user_id: str, page: int, n: int, order: PostsOrderType
    ) -> List[Post]:
    ...
    async def get_user_liked_posts(
        ...
        actions_select = await self.__session.execute(
            ...
            .where(
                and_(
                    PostActions.owner_id == user_id,
                    PostActions.action == ActionType.like,
                )
            )
            .offset(page * n)
            .limit(n)
    ...
    async def get_dialogue_by_users(
        self, user_1: User, user_2: User
    ) -> ChatRoom | None:
        result = await self.__session.execute(
            select(ChatRoom).where(
                and_(
                    ChatRoom.is_group == False,
                    ChatRoom.participants.contains(user_1),
                    ChatRoom.participants.contains(user_2),
                )
            )
        )
        return result.scalar()
    ...
    @postgres_exception_handler(
        action="Get n chat room messages excluding exclude_ids list"
    )
    async def get_chat_n_fresh_chat_messages(
        self,
        room_id: str,
        page: int,
        n: int = int(getenv("MESSAGES_BATCH_SIZE", "50")),
        pagination_normalization: int = 0,
    ) -> List[Message]:
        result = await self.__session.execute(
            select(Message)
            .where(Message.room_id == room_id)
    ...
    @postgres_exception_handler(
        action="Get n user chat rooms excluding exclude_ids list"
    )
    async def get_n_user_chats(
        self, user: User, n: int, page: int, approved: bool
    ) -> List[ChatRoom]:
    ...
    async def get_user_chats(self, user: User, approved: bool) -> ChatRoom:
        result = await self.__session.execute(
            select(ChatRoom).where(
                and_(
                    ChatRoom.participants.contains(user), ChatRoom.approved == approved
                )
            )
        )
    ...
    async def check_follow(self, user: User, other_user_id: str) -> bool:
        result = await self.__session.execute(
            select(User).where(
                and_(User.user_id == other_user_id, User.followers.contains(user))
            )
        )
