...
def define_database_url(mode: str) -> str:
    ...
    if mode == "prod":
        return f"postgresql+asyncpg://{getenv('DB_USERNAME')}:{getenv('DB_PASSWORD')}@{getenv('DB_HOST')}:{getenv('DB_PORT')}/{getenv('DB_NAME')}"
    elif mode == "test":
        return f"postgresql+asyncpg://{getenv('DB_USERNAME_TEST')}:{getenv('DB_PASSWORD_TEST')}@{getenv('DB_HOST_TEST')}:{getenv('DB_PORT_TEST')}/{getenv('DB_NAME_TEST')}"
...
async def create_engine(mode: str = "prod", echo=False) -> AsyncEngine:
    ...
    for i in range(RETRIES):
        try:
            ...
            engine = create_async_engine(url=DATABASE_URL, echo=echo)
