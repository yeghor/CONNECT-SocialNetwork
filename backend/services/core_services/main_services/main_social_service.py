...
HISTORY_POSTS_TO_TAKE_INTO_RELATED = int(
    getenv("HISTORY_POSTS_TO_TAKE_INTO_RELATED", 30)
)
...
MAX_REPLIES_THAT_GIVE_POPULARITY_RATE = int(
    getenv("MAX_REPLIES_THAT_GIVE_POPULARITY_RATE", "3")
)
...
class MainServiceSocial(MainServiceBase):
    @staticmethod
    def change_post_rate(
        post: Post, action_type: ActionType | None, add: bool, cost: int | None = None
    ) -> None:
    ...
    def check_post_user_id(post: Post, user: User) -> None:
        ...
        if post.owner_id != user.user_id:
            raise Unauthorized(
                detail=f"SocialService: User: {user.user_id} tried to access post: {post.post_id}",
                client_safe_detail="You are not owner of this post!",
            )

    async def _get_ids_by_query_type(
        self,
        page: int,
        user: User,
        n: int,
        id_type: Literal["followed", "fresh"],
        return_posts_too: bool = False,
        exclude_ids: List[str] = [],
    ) -> Union[List[str], NamedTuple]:
        if id_type == "fresh":
            posts = await self._PostgresService.get_fresh_posts(
                user=user, exclude_ids=exclude_ids, n=n, page=page
            )
        elif id_type == "followed":
            posts = await self._PostgresService.get_followed_posts(
                user=user, exclude_ids=exclude_ids, n=n, page=page
            )
    ...
    async def _construct_and_flush_action(
        self, action_type: ActionType, user: User, post: Post = None
    ) -> bool:
        """Returns `True` if action registered, otherwise `False`"""
        actions = await self._PostgresService.get_actions(
            user_id=user.user_id, post_id=post.post_id, action_type=action_type
        )
        cost = POST_ACTIONS[action_type.value]

        if actions:
            if action_type == ActionType.view:
                if not await self._RedisService.check_view_timeout(
                    id_=post.post_id, user_id=user.user_id
                ):
                    return False
                ...
                if len(actions) < MAX_REPLIES_THAT_GIVE_POPULARITY_RATE:
                    ...
                    for _ in range(len(actions)):
                        cost *= REPLY_COST_DEVALUATION
                else:
                    cost = 0
            else:
                raise InvalidAction(
                    detail=f"SocialService: User: {user.user_id} tried to give already given action: {action_type.value} to post: {post.post_id} that does not exists.",
                    client_safe_detail="This action is already given to this post.",
                )
    ...
    @web_exceptions_raiser
    async def get_feed(self, user: User | None, page: int) -> List[PostLiteSchema]:
        """`
        ...
        if not user:
            posts = await self._PostgresService.get_fresh_posts(
                user=None, page=page, n=BASE_PAGINATION, exclude_ids=[]
            )
        else:
            EACH_SOURCE_PAGINATION = int(BASE_PAGINATION / DIVIDE_BASE_PAG_BY)

            views_history = await self._PostgresService.get_user_actions(
                user_id=user.user_id,
                action_type=ActionType.view,
                n_most_fresh=HISTORY_POSTS_TO_TAKE_INTO_RELATED,
                return_posts=True,
            )
            liked_history = await self._PostgresService.get_user_actions(
                user_id=user.user_id,
                action_type=ActionType.like,
                n_most_fresh=LIKED_POSTS_TO_TAKE_INTO_RELATED,
                return_posts=True,
            )
            ...
            # History related mix
            if (
                len(views_history) > MINIMUM_USER_HISTORY_LENGTH
                and len(liked_history) > MINIMUM_USER_HISTORY_LENGTH
            ):
                related_ids = await self._ChromaService.get_n_related_posts_ids(
                    user=user,
                    page=page,
                    post_relation=history_posts_relation,
                    pagination=EACH_SOURCE_PAGINATION,
                )

                # Following mix
                followed_ids = await self._get_ids_by_query_type(
                    exclude_ids=related_ids,
                    user=user,
                    page=page,
                    n=EACH_SOURCE_PAGINATION,
                    id_type="followed",
                )
                if not followed_ids:
                    followed_ids = await self._get_ids_by_query_type(
                ...
                unrelevant_ids = await self._get_ids_by_query_type(
                    exclude_ids=followed_ids + related_ids,
                    user=user,
                    page=page,
                    n=EACH_SOURCE_PAGINATION,
                    id_type="fresh",
                ...
                related_ids = await self._get_ids_by_query_type(
                    page=page, user=user, n=EACH_SOURCE_PAGINATION, id_type="fresh"
                )
            ...
            posts = await self._PostgresService.get_entries_by_ids(
                ids=all_ids, ModelType=Post
            )
        ...
        return [
            PostLiteSchema(
                ...
                pictures_urls=await self._ImageStorage.get_post_image_urls(
                    images_names=[post_image.image_name for post_image in post.images]
                ),
                parent_post=(
                    PostBase(
                        ...
                        pictures_urls=await self._ImageStorage.get_post_image_urls(
                            images_names=[
                                ...
                                for post_image in post.parent_post.images
                            ]
                        ),
                    )
                    if post.parent_post
    ...
    async def get_followed_posts(self, user: User, page: int) -> List[PostLiteSchema]:
        post_ids, posts = await self._get_ids_by_query_type(
            page=page,
            n=BASE_PAGINATION,
            user=user,
            id_type="followed",
            return_posts_too=True,
        )
    ...
    async def search_posts(
        self, query: str, user: User | None, page: int
    ) -> List[PostLiteSchema]:
        """
        Search posts that similar with meaning to prompt
        """

        post_ids = await self._ChromaService.search_posts_by_prompt(
            query=query, page=page, n=BASE_PAGINATION
        )
        posts = await self._PostgresService.get_entries_by_ids(
            ids=post_ids, ModelType=Post
        )
        ...
        return [
            PostLiteSchema(
                ...
                views=post.views_count,
                replies=post.replies_count,
                is_my_post=user.user_id == post.post_id if user else False,
                owner=(
                    UserShortSchemaAvatarURL(
                        user_id=post.owner_id,
                        username=post.owner.username,
                ...
                parent_post=(
                    PostBase(
                        ...
                        owner=(
                            UserShortSchemaAvatarURL(
                                user_id=post.parent_post.owner.owner_id,
                                username=post.parent_post.owner.username,
                                avatar_url=await self._ImageStorage.get_user_avatar_url(
                                    post.parent_post.owner_id
                                ),
                        ...
                        replies=post.parent_post.replies_count,
                        is_my_post=(
                            user.user_id == post.parent_post.post_id if user else False
                        ),
                        pictures_urls=await self._ImageStorage.get_post_image_urls(
                            images_names=[
                                post_image.image_name
    ...
    @web_exceptions_raiser
    async def search_users(self, query: str, page: int) -> List[UserLiteSchema]:
        users = await self._PostgresService.get_users_by_username(
            query=query, page=page, n=BASE_PAGINATION
        )
        return [
            UserLiteSchema(
                ...
                joined=user.joined,
                avatar_url=await self._ImageStorage.get_user_avatar_url(user.user_id),
            )
            for user in users
        ]
    ...
    async def remove_action(
        self, user: User, post: Post, action_type: ActionType
    ) -> None:
        if action_type.value == "reply":
            raise InvalidAction()

        potential_action = await self._PostgresService.get_actions(
            user_id=user.user_id, post_id=post.post_id, action_type=action_type
        )
        if not potential_action:
            raise InvalidAction(
                detail=f"SocialService: User: {user.user_id} tried to remove post action: {post.post_id} that does not exists."
            )

        if len(potential_action) > 1:
            raise WrongDataFound(
                detail=f"SocialService: User: {user.user_id} tried to remove post action, but multiple instances of action",
                client_safe_detail="Something wrong happened on our side, please, contact us.",
            )
    ...
    async def delete_post(self, post_id: str, user: User) -> None:
        ...
        if not post:
            raise ResourceNotFound(
                detail=f"SocialService: User: {user.user_id} tried to delete post: {post_id} that does not exist.",
                client_safe_detail="Post that you trying to delete does not exist.",
            )
    ...
    @web_exceptions_raiser
    async def like_post_action(
        self, post_id: str, user: User, like: bool = True
    ) -> None:
        """Set 'like' param to True to leave like. To remove like - set to False"""
        post = await self._PostgresService.get_entry_by_id(id_=post_id, ModelType=Post)
        if like:
            await self._construct_and_flush_action(
                action_type=ActionType.like, post=post, user=user
            )
            post.likes_count += 1
        else:
    ...
    async def change_post(
        self, post_data: PostDataSchemaID, user: User, post_id: str
    ) -> PostSchema:
        post = await self._PostgresService.get_entry_by_id(id_=post_id, ModelType=Post)

        if not post:
            raise ResourceNotFound(
                detail=f"SocialService: User: {user.user_id} tried to change post: {post_id} that does not exist.",
                client_safe_detail="Post that you trying to change does not exist.",
            )

        self.check_post_user_id(post=post, user=user)

        updated_post = await self._PostgresService.update_post_fields(
            post_data=post_data, post_id=post_id, return_updated_post=True
        )
    ...
    @web_exceptions_raiser
    async def friendship_action(
        self, user: User, other_user_id: str, follow: bool
    ) -> None:
        """To follow user - set follow to True. To unfollow - False"""

        if user.user_id == other_user_id:
            raise InvalidAction(
                detail=f"SocialService: User: {user.user_id} tried to follow himself.",
                client_safe_detail="You can't follow yourself.",
            )

        other_user = await self._PostgresService.get_entry_by_id(
            id_=other_user_id, ModelType=User
        )

        # Getting fresh user. Because merged Model often lose it's relationships loads
        fresh_user = await self._PostgresService.get_entry_by_id(
            id_=user.user_id, ModelType=User
        )

        if follow:
            if other_user in fresh_user.followed:
                raise InvalidAction(
                    detail=f"SocialService: User: {user.user_id} tried to follow user: {other_user.user_id} already following him",
                    client_safe_detail="You are already following this user. You can't follow him",
                )
            fresh_user.followed.append(other_user)
        elif not follow:
            if other_user not in fresh_user.followed:
                raise InvalidAction(
                    detail=f"SocialService: User: {user.user_id} tried to unfollow user: {other_user.user_id} not following him",
                    client_safe_detail="You are not following this user. You can't unfollow him",
                )
            fresh_user.followed.remove(other_user)

    @web_exceptions_raiser
    async def get_user_profile(
        self, user: User | None, other_user_id: str
    ) -> UserSchema:
        other_user = await self._PostgresService.get_entry_by_id(
            id_=other_user_id, ModelType=User
        )
        ...
        if not other_user:
            raise ResourceNotFound(
                detail=f"User: {user.user_id} tried to get user: {other_user_id} profile that does not exist.",
                client_safe_detail="User profile that you trying to get does not exist.",
            )

        avatar_token = await self._ImageStorage.get_user_avatar_url(
            image_name=other_user.user_id
        )
        ...
        return UserSchema(
            ...
            me=user.user_id == other_user_id if user else False,
            is_following=(
                await self._PostgresService.check_follow(
                    user=user, other_user_id=other_user_id
                )
                if user
                else False
            ),
    ...
    async def get_my_profile(self, user: User) -> UserSchema:
        ...
        user = await self._PostgresService.get_entry_by_id(
            id_=user.user_id, ModelType=User
        )
    ...
    @web_exceptions_raiser
    async def load_post(self, user: User | None, post_id: str) -> PostSchema:
        post = await self._PostgresService.get_entry_by_id(id_=post_id, ModelType=Post)

        if not post:
            raise ResourceNotFound(
                detail=f"SocialService: User: {user.user_id} tried to load post: {post_id} that does not exist.",
                client_safe_detail="This post does not exist.",
            )

        if user:
            registered_view = await self._construct_and_flush_action(
                action_type=ActionType.view, post=post, user=user
            )
    ...
    async def load_replies(
        self, user: User | None, post_id: str, page: int
    ) -> List[PostBase]:
        replies = await self._PostgresService.get_post_replies(
            post_id=post_id, page=page, n=SMALL_PAGINATION
        )
    ...
    async def _get_recent_action_message(
        self, action: PostActions
    ) -> RecentActivitySchema | None:
        """
    ...
    async def get_recent_activity(
        ...
        actions = await self._PostgresService.get_fresh_actions_for_user(
            user, RECENT_ACTIVITY_ENTRIES
        )
