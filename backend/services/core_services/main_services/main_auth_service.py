...
class MainServiceAuth(MainServiceBase):
    async def __generate_set_of_tokens(
        self,
        user_id: str,
        email_confirmation_required: bool = False,
        check_tokens_exist: bool = True,
    ) -> RefreshAccessTokens:
        potential_refresh_token = await self._RedisService.get_token_by_user_id(
            user_id=user_id, token_type="refresh"
        )
        potential_access_token = await self._RedisService.get_token_by_user_id(
            user_id=user_id, token_type="acces"
        )

        if check_tokens_exist:
            if potential_access_token:
                await self._RedisService.delete_jwt(
                    jwt_token=potential_access_token, token_type="acces"
                )
            if potential_refresh_token:
                await self._RedisService.delete_jwt(
                    jwt_token=potential_refresh_token, token_type="refresh"
                )
    ...
    async def __create_2fa(self, email: str, username: str) -> None:
        ...
        await self._RedisService.assign_second_factor(
            email=email, code=confirmation_code
        )

    @web_exceptions_raiser
    async def authorize_token_and_optionally_return_user(
        self, token: str, token_type: EndpointAuthType, return_user: bool = True
    ) -> User | None:
        ...
        if not await self._RedisService.check_jwt_existence(
            ...
            raise Unauthorized(
                detail=f"AuthService: User tried to authorize request by expired token, token type: {token_type}",
        ...
        if return_user:
            ...
            if not user:
                raise Unauthorized(
                    detail=f"AuthService: User tried to authorize request by token: {token} token type: {token_type}, but specified user id does not exist.",
                    client_safe_detail="Invalid or expired token",
                )
            return user
    ...
    async def authorize_chat_token(self, token: str) -> str:
        ...
        if not await self._RedisService.check_chat_token_existense(chat_token=token):
            raise UnauthorizedInWebsocket(
                dev_log_detail=f"AuthService: User tried to connected to th ws chat by expired chat token: {token}",
                client_safe_detail="Invalid or expired token",
            )
    ...
    async def register(self, credentials: RegisterBody) -> EmailProvided:
        ...
        if await self._PostgresService.get_user_by_username_or_email(
            ...
            raise Collision(
                detail=f"AuthService: User tried to register with credentials: {credentials.username}, {credentials.email} that already exist.",
                client_safe_detail="Registered user with these credentials already exist",
            )
    ...
    async def login(self, credentials: LoginBody) -> RefreshAccessTokens:
        ...
        if not potential_user:
            raise InvalidResourceProvided(
                ...
                client_safe_detail="Account with these credentials does not exist. You may need to sign up first",
            )

        if not authorization_utils.check_password(
            credentials.password, potential_user.password_hash
        ):
            raise Unauthorized(
                detail=f"AuthService: User with: {credentials.username} username tried to login with wrong password.",
                client_safe_detail="Password didn't match",
            )

        if not potential_user.email_confirmed:
            # Returning null tokens with email confirmation required flag set to True
            await self.__create_2fa(
                email=potential_user.email, username=potential_user.username
            )
    ...
    async def confirm_email_2fa(
        self, credentials: _2FAConfirmationBody
    ) -> RefreshAccessTokens:
        if not await self._RedisService.check_2fa(
            email=credentials.email, code=credentials.confirmation_code
        ):
            raise InvalidResourceProvided(
                detail=f"AuthService: User with email: {credentials.email} tried to perform 2fa using wrong code.",
                client_safe_detail="Second factor authentication failed",
            )

        await self._RedisService.deactivate_second_factor(email=credentials.email)

        confirmed_user = await self._PostgresService.get_user_by_username_or_email(
            email=credentials.email
        )

        if not confirmed_user:
            raise InvalidResourceProvided(
                detail=f"AuthService: user with email: {credentials.email} tried to confirm email, but such user doesn't exist yet",
                client_safe_detail=f"Email that you provided doesn't exist in our system",
            )
    ...
    async def recover_password_2fa(
        ...
        user = await self._PostgresService.get_user_by_username_or_email(
            email=credentials.email
        )

        if not user:
            raise InvalidResourceProvided(
                detail=f"AuthService: user with email: {credentials.email} tried to get password recovery token, but such user doesn't exist yet",
                client_safe_detail=f"Email that you provided doesn't exist in our system",
            )
        elif not user.email_confirmed:
            raise InvalidAction(
                detail=f"AuthService: User: {user.user_id} with email: {user.email} tried to confir 2fa password recovery, while his email wasn't confirmed",
                client_safe_detail="You must confirm your email first!",
            )

        return await self._JWT.generate_save_token(
            user_id=user.user_id,
            redis=self._RedisService,
            token_type="password-recovery",
        )

    @web_exceptions_raiser
    async def recover_password(
        self, user: User, credentials: PasswordRecoveryBody
    ) -> RefreshAccessTokens:
        ...
        return await self.__generate_set_of_tokens(
            user_id=user.user_id, email_confirmation_required=False
        )
    ...
    async def change_password(
        self, user: User, credentials: ChangePasswordBody
    ) -> RefreshAccessTokens:
        authorization_utils.validate_password(credentials.new_password)

        if not authorization_utils.check_password(
            credentials.old_password, user.password_hash
        ):
            raise InvalidResourceProvided(
                detail=f"AuthService: User: {user.user_id} tried to change password using wrong old password.",
                client_safe_detail="Old password didn't match",
            )
        ...
        return await self.__generate_set_of_tokens(
            user.user_id, email_confirmation_required=False, check_tokens_exist=False
        )
    ...
    async def logout_on_this_device(self, tokens: RefreshAccessTokens) -> None:
        await self._RedisService.delete_jwt(
            jwt_token=tokens.access_token, token_type="acces"
        )
        await self._RedisService.delete_jwt(
            jwt_token=tokens.refresh_token, token_type="refresh"
        )
    ...
    async def refresh_token(self, refresh_token: str) -> AccessTokenSchema:
        ...
        if not await self._RedisService.check_jwt_existence(
            ...
            raise Unauthorized(
                detail=f"AuthService: User with refresh token: {refresh_token} that does not exist tried to refresh tokens.",
                client_safe_detail="Invalid or expired token",
            )
        ...
        if not await self._PostgresService.get_user_by_id(user_id=user_id):
            ...
            raise Unauthorized(
                detail=f"AuthService: user: {user_id} tried to refresh access token, while his profile instance was deleted.",
                client_safe_detail="Invalid or expired token",
            )

        old_access_token = await self._RedisService.get_token_by_user_id(
            user_id=user_id, token_type="acces"
        )
        new_access_token = await self._JWT.generate_save_token(
            user_id=user_id, redis=self._RedisService, token_type="acces"
        )

        await self._RedisService.refresh_access_token(
            old_token=old_access_token,
            new_token=new_access_token.access_token,
            user_id=user_id,
        )
    ...
    async def change_username(
        self, user: User, credentials: NewUsernameBody
    ) -> EmailProvided:
        new_username = credentials.new_username

        if user.username == credentials.new_username:
            raise InvalidResourceProvided(
                detail=f"AuthService: User: {user.user_id} tried to change username to identical to his old one.",
                client_safe_detail="New username can't the same as old one",
            )
    ...
    async def delete_user(self, password: str, user: User) -> None:
        if not authorization_utils.check_password(
            ...
            raise InvalidResourceProvided(
                detail=f"AuthService: User: {user.user_id} tried to delete his profile, but password didn't match.",
                client_safe_detail="Password didn't match",
            )
