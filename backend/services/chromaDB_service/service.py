...
class ChromaService:
    ...
    def extract_ids_from_metadata(metadatas, page: int, pagination: int) -> List[str]:
        # TODO: !!!!!!!
        all_ids = [
            str(meta["post_id"]) for batch in metadatas["metadatas"] for meta in batch
        ]
    ...
    async def drop_all(self):
        ...
        if self.__mode == "prod":
            ...
            self.__collection = await self.__client.create_collection(
                name=PROD_COLLECTION_NAME
            )
    ...
    @chromaDB_error_handler
    async def get_n_related_posts_ids(
        self, user: User, page: int, post_relation: List[Post], pagination: int
    ) -> List[str]:
        ...
        related_posts_metadatas = await self.__collection.query(
            query_texts=[
                f"{post.title} {post.text} {post.published.strftime(self._datetime_format)}"
                for post in post_relation
            ],
            n_results=(
                (pagination * page) + pagination + GET_EXTRA_CHROMADB_RELATED_RESULTS
            ),
        )

        return self.extract_ids_from_metadata(
            metadatas=related_posts_metadatas, page=page, pagination=pagination
        )
            ...
            metadatas=[
                {
                    ...
                    "published": int(post.published.timestamp()),
    ...
    async def search_posts_by_prompt(self, query: str, page: int, n: int) -> List[str]:
        ...
        return self.extract_ids_from_metadata(
            metadatas=search_result, page=page, pagination=n
        )

    @chromaDB_error_handler
    async def delete_by_ids(self, ids: List[str]):
        await self.__collection.delete(ids=ids)
