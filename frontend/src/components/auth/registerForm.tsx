...
import {
    appHomeURI, internalServerErrorURI, appLoginURI,
...
const RegisterForm = () => {
    ...
    const formHandler = async (event: React.FormEvent): Promise<void> => {
        ...
        } else if(!validateFromString(email, "email")) {
    ...
    return (
        <section className="bg-gray-50 dark:bg-gray-900">
            <div className="flex flex-col items-center justify-top px-6 py-8 mt-16 mx-auto md:h-screen lg:py-0">
                <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                    <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form onSubmit={formHandler} className="space-y-4 md:space-y-6" action="#">
                            ...
                            <button
                                type="submit"
                                className="w-full text-white bg-black focus:ring-4 focus:outline-1 focus:ring-zinc-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-zinc-600 dark:hover:bg-zinc-700 dark:focus:ring-zinc-800">
                                Sign up
                            </button>
                            <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                                Already have an account? <Link to={appLoginURI} className="font-medium text-primary-600 text-black hover:underline dark:text-primary-500">Sign up</Link>
                            </p>
                        </form>
                    </div>
                </div>
